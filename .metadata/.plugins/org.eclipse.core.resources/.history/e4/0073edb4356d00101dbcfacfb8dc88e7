...
public class EmpRepository implements IEmpRepository {
    ...
    public void updateEmp(EmpVO emp) {
        ...
        jdbcTemplate.update(sql,
            ...
            emp.getEmail(),
            emp.getPhoneNumber(),
            emp.getHireDate(),       // java.sql.Date or java.util.Date
            emp.getJobId(),
            emp.getSalary(),
            emp.getCommissionPct(),
            emp.getManagerId(),
    ...
    public int deleteEmp(int empId, String email) {
        // 0. manager_id로 이 사원을 참조하는 다른 직원이 있다면 manager_id NULL로 업데이트
        String updateManagerQuery = "UPDATE employees SET manager_id = NULL WHERE manager_id = ?";
        jdbcTemplate.update(updateManagerQuery, empId);

        // 1. 해당 사원이 관리자인 부서도 처리
